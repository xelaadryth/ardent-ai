"""
Vault module - provides vault operations split by domain.

This module re-exports the public API from vault_index.py for backward compatibility.
"""

# Import from file_io module
from .file_io import (
    AI_FOLDER,
    CORE_FILES,
    VAULT_ROOT,
    load_core_context,
    load_markdown,
    read_file,
    write_file,
)

# Re-export from submodules for backward compatibility
from .io import get_index_file, load_vault_index, save_vault_index
from .parser import build_index_entry, extract_wikilinks, parse_frontmatter
from .crawler import build_index_from_disk, crawl_numbered_markdown_files
from .retrieval import retrieve_vault_context
from .mapping import (
    get_folder_from_type,
    get_folder_prefix_for_type,
    get_filepath_from_name,
    get_filepath_from_name_and_type,
)
from .score import (
    normalize_text,
    query_terms,
    score_entry,
    sort_by_score,
)

__all__ = [
    # From file_io module
    "AI_FOLDER",
    "VAULT_ROOT",
    "CORE_FILES",
    "read_file",
    "write_file",
    "load_markdown",
    "load_core_context",
    # I/O
    "get_index_file",
    "load_vault_index",
    "save_vault_index",
    # Parsing
    "parse_frontmatter",
    "build_index_entry",
    "extract_wikilinks",
    # Crawling
    "crawl_numbered_markdown_files",
    "build_index_from_disk",
    # Retrieval
    "retrieve_vault_context",
    # Mapping
    "get_folder_prefix_for_type",
    "get_folder_from_type",
    "get_filepath_from_name_and_type",
    "get_filepath_from_name",
    # Utilities
    "normalize_text",
    "query_terms",
    "score_entry",
    "sort_by_score",
]
